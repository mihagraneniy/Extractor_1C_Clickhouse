...
Процедура ЗагрузитьНастройкиОбработкиИзФайла(Команда)
	Если ЗначениеЗаполнено(ПутьКФайлуНастройки) Тогда
		...
		ЗагрузитьНастройкиОбработкиИзФайлаНаСервере(АдресВХ);
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура ЗагрузитьНастройкиОбработкиИзФайлаНаСервере(АдресВХ)
...
Процедура ПериодПриИзменении(Элемент)
	...
	Если Не Объект.ЭтоТекущаяДата Тогда
		Объект.КоличествоДнейНазад = (НачалоДня(Объект.ДатаОкончания) - НачалоДня(Объект.ДатаНачала)) / (60 * 60 * 24);
	КонецЕсли;
КонецПроцедуры
