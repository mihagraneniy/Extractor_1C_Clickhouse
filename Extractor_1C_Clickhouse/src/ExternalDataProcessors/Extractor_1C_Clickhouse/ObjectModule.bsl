...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.3.2");
...
Процедура ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, ПараметрыЗапроса, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнительныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	КонецЕсли;
	РезультатИндексацииПоСКД = ВернутьИндекациюПоСКДОписаниеТаблицыБД(Параметры.ОписаниеТаблицыБД, ВыборкаРезультата.Колонки, Параметры);
	...
	Если ЕстьРазница И Параметры.ПроверитьТаблицу Тогда
		...
		КонецЦикла;
		Сообщить(Сообщение);
		Если Не Параметры.СоздаватьНесуществующиеПоляВБД Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	//
	Если Параметры.СоздаватьНесуществующиеПоляВБД Тогда
		//
		НайденныйСтроки = ИндексацияТЗ.НайтиСтроки(Новый Структура("ЕстьРазница", Истина));
		Для каждого СтрокаН из НайденныйСтроки Цикл
...
Функция ВернутьИндекациюПоСКДОписаниеТаблицыБД(ВходящаяТЗ, ЗапросКолонки, Параметры)
	...
	МассивПолейЗапрос = Новый Массив;
	Для каждого СтрокаК из ЗапросКолонки Цикл
		МассивПолейЗапрос.Добавить(СтрокаК.Имя);	 
	КонецЦикла;
	...
	Если КоличествоВходящаяТЗ <> КоличествоМассивПолейЗапрос Тогда
		Если КоличествоВходящаяТЗ > КоличествоМассивПолейЗапрос Тогда
			Для каждого СтрокаТЗ из ВходящаяТЗ Цикл
				...
				Иначе
					...
					НоваяСтрокаТЗ.ИмяД = СтрокаТЗ.Имя;
		...
		Иначе
			Для каждого КолонкаЗ из ЗапросКолонки Цикл
				...
				//НоваяСтрокаТЗ.СозданиеПоля = "ALTER TABLE " + Параметры.ИмяБазы + "." + Параметры.ИмяТаблицыТранслит + " ADD COLUMN `" + ТранслитСтрокаМ + "` String;";
	...
	Иначе
		Для каждого КолонкаЗ из ЗапросКолонки Цикл
			...
			НайденнаяСтрока = ВходящаяТЗ.Найти(ТранслитСтрокаМ, "Имя");
			//Если НайденнаяСтрока = Неопределено Тогда
			//	НайденнаяСтрока = НайденнаяСтрока = ВходящаяТЗ.Найти(ТранслитСтрокаМ, "Имя");
			//КонецЕсли;
			Если НайденнаяСтрока = Неопределено Тогда 
				НоваяСтрокаТЗ = РезультатТЗ.Добавить();
				НоваяСтрокаТЗ.ЕстьРазница = Истина;
