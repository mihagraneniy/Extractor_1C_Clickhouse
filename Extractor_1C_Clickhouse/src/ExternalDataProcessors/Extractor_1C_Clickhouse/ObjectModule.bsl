...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.1.5");
...
Функция ПолучитьОсобыеПараметры(ДополнительныеПараметры, ПараметрыМакета)
	СтруктураПараметров = Новый Структура("Передичность, ЕстьОсобоеПолеДатыОкончениеПериода");
	...
	Если ДополнительныеПараметры.КоличествоДнейНазад > 0 Тогда
		...
		Для Счетчик = 1 По ДополнительныеПараметры.КоличествоДнейНазад + 1 Цикл
			...
			Для каждого СтрокаПараметра из ПараметрыМакета Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ЗаписатьРезультатЗапросаClickHouseНаСервере(Запрос, Параметры, НоваяДатаОтчета, ПоляНабораДанных, МассивПолей, ДополнительныеПараметры);
			//
			НоваяДатаОтчета = ДатаДнейВперед(НоваяДатаОтчета, 1);
			Если НоваяДатаОтчета > Параметры.ДатаОкончания Тогда
				Прервать;
			КонецЕсли;
			Если НоваяДатаОтчета = Параметры.ДатаНачала Тогда
				Прервать;
			КонецЕсли;
		КонецЦикла;
	Иначе
		ЗаписатьРезультатЗапросаClickHouseНаСервере(Запрос, Параметры, Параметры.ДатаНачала, ПоляНабораДанных, МассивПолей, ДополнительныеПараметры);
	КонецЕсли;	 
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 27-10-2023 9:30 start																						 
Процедура ЗаписатьРезультатЗапросаClickHouseНаСервере(Запрос, Параметры, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнительныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	//
	//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 14-03-2024 19:30 start
	РезультатИндексацииПоСКД = ВернутьИндекациюПоСКДОписаниеТаблицыClickHouse(Параметры.ОписаниеТаблицыClickHouse, МассивПолейЗапрос, Параметры);
	...
	Если ДополнительныеПараметры.ОднимФайлом Тогда
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз ИЛИ ИндексЗаписи = КоличествоЗаписей Тогда
				...
				//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 28-03-2024 19:30 start
				Если РезультатВыполненияЗапроса Тогда
					ТекстЗапроса = "OPTIMIZE TABLE " + Параметры.ИмяБазы + "." + Параметры.ИмяТаблицыТранслит +" FINAL";
					РезультатВыполненияЗапроса = ВыполнитьЗапросPOST_ClickHouse(ТекстЗапроса + ";", Параметры);
				КонецЕсли;
				//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 28-03-2024 19:30 end
				КоличествоСтрокОбработано = 0;
				ЗначенияЗапроса = "";
			Иначе
				//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 30-05-2024 19:30 start
	...
	Иначе
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если Счетчик >= КоличествоСтрокМаксЗаРаз Тогда
				...
				//
				ЗапуститьФоновоеЗаданиеClickHouse(Параметры, ПериодДата, МассивПолей, ДополнительныеПараметры, ТЗ, МассивПолейЗапрос, РезультатИндексацииПоСКД);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 14-03-2024 19:30 start
Функция ВернутьИндекациюПоСКДОписаниеТаблицыClickHouse(ТЗ, МассивПолейЗапрос, Параметры)
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 16-01-2023 18:00 start
Процедура ЗапуститьФоновоеЗаданиеClickHouse(Параметры, ПериодДата, МассивПолей = Неопределено, ДополнительныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 01-11-2023 9:30 start	 
Функция ВернутьМассивПорядокПериодИзРезультатаЗапроса(РезультатЗапроса, ДополнительныеПараметры)
	М = Новый Массив;
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 03-11-2023 9:30 start
Функция ВернутьМассивПорядокПериодИзНабораДанных(ПоляНабораДанных, ДополнительныеПараметры)
	М = Новый Массив;
	Если ДополнительныеПараметры.ИспользоватьПериодВКлюче Тогда
		ИмяПериод = "Период"; 	
		М.Добавить(ИмяПериод);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 03-11-2023 9:30 start			 
Функция ВернутьМассивПолейКлючейПериодИзНабораДанных(ПоляНабораДанных, ДополнительныеПараметры)
	М = Новый Массив;
	Если ДополнительныеПараметры.ИспользоватьПериодВКлюче Тогда
		ИмяПериод = "Период"; 	
		М.Добавить(ИмяПериод);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 01-11-2023 9:30 start									   
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанных(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры)
