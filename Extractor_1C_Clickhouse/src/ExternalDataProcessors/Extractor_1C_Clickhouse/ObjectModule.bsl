...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.2.2");
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз ИЛИ ИндексЗаписи = КоличествоЗаписей Тогда
				...
				ИначеЕсли Параметры.ТипБазы = "MySQL" Тогда
					...
					СтрокаКолонкиДляUpdate = ПолучитьКолонкиДляUpdateMySQL(МассивПолей);
					//ТекстЗапроса = ТекстЗапроса + " `" + Транслит("ДатаОбновленияЗаписи") + "` = NOW();";
...
Функция ВернутьИндекациюПоСКДОписаниеТаблицыБД(ВходящаяТЗ, МассивПолейЗапрос, Параметры)
	...
	Для каждого СтрокаМ из МассивПолейЗапрос Цикл
		...
		Отбор.Вставить("Имя", ТранслитСтрокаМ);
		НайденныеСтроки = ВходящаяТЗ.НайтиСтроки(Отбор);
		Если НайденныеСтроки.Количество() <= 0 Тогда
			Отбор.Вставить("Имя", НРег(ТранслитСтрокаМ));
			НайденныеСтроки = ВходящаяТЗ.НайтиСтроки(Отбор);	
		КонецЕсли;
		Если НайденныеСтроки.Количество() <= 0 Тогда
			НоваяСтрока = РезультатТЗ.Добавить();
			НоваяСтрока.ЕстьРазница = Истина;
			Результат.Вставить("ЕстьРазница", Истина);
		КонецЕсли;
		Для каждого СтрокаТЗ из НайденныеСтроки Цикл
			НоваяСтрока = РезультатТЗ.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЗ);
			НоваяСтрока.ЕстьРазница = Ложь;
			Результат.Вставить("ЕстьРазница", Ложь);
		КонецЦикла;
		НоваяСтрока.ИмяСКД = СтрокаМ;
		НоваяСтрока.ИндексСКД = Индекс;
		//
		НоваяСтрока.СозданиеПоля = "ALTER TABLE " + Параметры.ИмяБазы + "." + Параметры.ИмяТаблицыТранслит + " ADD COLUMN `" + ТранслитСтрокаМ + "` String;"; 
		//
		Индекс = Индекс + 1;
	КонецЦикла;
	//
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 10-04-2025 9:30 start								  
Функция ПолучитьКлючИзМассива(МассивКлючей, Префикс = "", ДлинаИндексаБайт = 0, ДлинаЗначенияДляИндекса = 0)
	КлючМ = Префикс + ПолучитьЗначениеИндексаСДлинойБайт(МассивКлючей[0], ДлинаЗначенияДляИндекса);
		...
		Если Счетчик > 1 Тогда
			Если ДлинаИндексаБайт > 0 Тогда
				Если СчетчикДлинаИндексаБайт + ДлинаИндексаБайтТекущая > ДлинаИндексаБайт Тогда
					Прервать;
				Иначе
					КлючМ = ПолучитьДополненныйКлючМ(КлючМ, Префикс, КлючС, ДлинаЗначенияДляИндекса);	
				КонецЕсли;
			Иначе
				КлючМ = ПолучитьДополненныйКлючМ(КлючМ, Префикс, КлючС, ДлинаЗначенияДляИндекса);
			КонецЕсли;
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 11-04-2025 9:30 start								  
Функция ПолучитьКолонкиДляUpdateMySQL(МассивКлючей)
	СтрокаКолонки = "" + Транслит(МассивКлючей[0]) + " = " + Транслит(МассивКлючей[0]);
	
	Для каждого СтрокаМ из МассивКлючей Цикл
		СтрокаКолонки = СтрокаКолонки + ", " + Транслит(СтрокаМ) + " = " + Транслит(СтрокаМ);
	КонецЦикла;
