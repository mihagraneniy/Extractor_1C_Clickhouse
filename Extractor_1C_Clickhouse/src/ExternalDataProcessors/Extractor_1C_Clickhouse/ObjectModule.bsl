...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.1.7");
	...
	Если ДополнительныеПараметры.КоличествоДнейНазад > 0 Тогда
		...
		Для Счетчик = 1 По ДополнительныеПараметры.КоличествоДнейНазад + 1 Цикл
			...
			Для каждого СтрокаПараметра из ПараметрыМакета Цикл
				...
				КонецЕсли;
			КонецЦикла;
			ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, НоваяДатаОтчета, ПоляНабораДанных, МассивПолей, ДополнительныеПараметры);
			//
		...
		КонецЦикла;
	Иначе
		ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, Параметры.ДатаНачала, ПоляНабораДанных, МассивПолей, ДополнительныеПараметры);
	КонецЕсли;	 
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 27-10-2023 9:30 start																						 
Процедура ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнительныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	Если ДополнительныеПараметры.ОднимФайлом Тогда
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз ИЛИ ИндексЗаписи = КоличествоЗаписей Тогда
				...
				РезультатВыполненияЗапроса = ВыполнитьЗапросPOST(ТекстЗапроса + " VALUES " + ЗначенияЗапроса + ";", Параметры);
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 14-03-2024 19:30 start
Функция ВернутьИндекациюПоСКДОписаниеТаблицыБД(ТЗ, МассивПолейЗапрос, Параметры)
...
Функция ЕстьБазаВPostgreSQL(ИмяБазы, Параметры)
	ТекстЗапрос = "SELECT name, comment FROM system.databases WHERE name = '" + Параметры.ИмяБазы + "';";
	//
	ТекстОтвета = ВыполнитьЗапросGET_PostgreSQL(ТекстЗапрос, Параметры, "get");
	Если ТекстОтвета = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	Если ЭтоОшибкаЗапросаPostgreSQL(ТекстОтвета) Тогда
		Возврат Ложь;
	КонецЕсли;
	Если ТекстОтвета <> "" Тогда
		Возврат Истина;
...
Функция ТекстЗапросаСозданиеТаблицыПериодСхемаКомпоновкиДанныхPostgreSQL(Параметры, ПараметрыМакета, ПоляМакета, ПоляКлючи, ДополнительныеПараметры, ИмяПоляМ)
	...
	Если ДополнительныеПараметры.ИспользоватьПериодВКлюче Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + " (" + Транслит("Период") + " time without time zone DEFAULT '00:00:00'::time without time zone,";	
	Иначе
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + " (";
	КонецЕсли;
	Счетчик = 1;
	...
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + "CONSTRAINT "+ Параметры.ИмяТаблицыТранслит +"_pkey PRIMARY KEY (" + ПолучитьКлючИзМассива(ПоляКлючи) + ")";
	ТекстЗапроса = ТекстЗапроса + Символы.ПС + ")";
...
Функция ПолучитьТипПоляНабораPostgreSQL(ПолеНабора)
	...
	Если ТипЗнч(ПолеНабора.ТипЗначения) = Тип("ОписаниеТипов") Тогда
		...
		ИначеЕсли Строка(ПолеНабора.ТипЗначения) = "Дата" Тогда
			Возврат "time without time zone DEFAULT '00:00:00'::time without time zone";
		ИначеЕсли Строка(ПолеНабора.ТипЗначения) = "Строка" Тогда
