...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.3.6");
...
Процедура ЗаписатьРезультатЗапросаЭкстракторНаСервере(Запрос, Параметры, ПараметрыЗапроса, ПериодДата, ПоляНабораДанных = Неопределено, МассивПолей = Неопределено, ДополнительныеПараметры = Неопределено, ВходнаяТЗ = Неопределено, МассивПолейЗапрос = Неопределено, ИндексацияТЗ = Неопределено, ЕстьРазница = Неопределено)
	...
	Если Параметры.ТипБазы = "REST" Тогда
		Если ДополнительныеПараметры.ФорматДанных = "TSV" Тогда
		...
		Для каждого СтрокаР из ВыборкаРезультата Цикл
			...
			Если КоличествоСтрокОбработано = КоличествоСтрокМаксЗаРаз ИЛИ (ИндексЗаписи + 1) = КоличествоЗаписей Тогда
				...
				ИначеЕсли Параметры.ТипБазы = "REST" Тогда
					...
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "CSV" Тогда
						...
						ТекстЗапросаPOST = ТекстЗапроса + ЗначенияЗапроса + "";
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "JSON" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапросJSON(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
						ТекстЗапросаPOST = ТекстЗапроса + ЗначенияЗапроса + "]";
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "XML" Тогда
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "ClickHouseDB" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
			...
			Иначе
				...
				Если Параметры.ТипБазы = "REST" Тогда
					...
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "JSON" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапросJSON(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
						ЗначенияЗапроса = ЗначенияЗапроса + ",";
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "XML" Тогда
					ИначеЕсли ДополнительныеПараметры.ФорматДанных = "ClickHouseDB" Тогда
						ЗначенияЗапроса = ЗначенияЗапроса + ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница);
						ЗначенияЗапроса = ЗначенияЗапроса + "," + Символы.ПС;
...
Функция ВернутьЗначенияЗапрос(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница)
	...
	ТекстЗапросаПоля = "";
	Если Параметры.ТипБазы = "REST" Тогда
		Если Параметры.ФорматДанных = "TSV" Тогда
...
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 05-08-2025 9:30 start
Функция ВернутьЗначенияЗапросJSON(СтрокаР, Параметры, ПериодДата, МассивПолей, МассивПолейЗапрос, ИндексацияТЗ, ЕстьРазница)
	Счетчик = 0;
	ТекДата = ТекущаяДата();
	КоличествоПолей = ИндексацияТЗ.Количество();
	ТекстЗапросаПоля = "";
	...
	Для Каждого Элемент из СтруктураПолейЗапрос Цикл
		ЭлементЗначение = Элемент.Значение;
		ЭлементКлюч = Элемент.Ключ;
		Если ТипЗнч(ЭлементЗначение) = Тип("Дата") Тогда
			СтруктураПолей.Вставить(ЭлементКлюч, ЭлементЗначение);
		...
		ИначеЕсли ТипЗнч(ЭлементЗначение) = Тип("УникальныйИдентификатор") Тогда
			СтруктураПолей.Вставить(ЭлементКлюч, СокрЛП(Строка(ЭлементЗначение)));
		Иначе
			Если ЭлементЗначение <> Null И ЭлементЗначение <> Неопределено Тогда
				ТипЗначения = ТипЗнч(ЭлементЗначение);
				ОбъектМетаданных = Метаданные.НайтиПоТипу(ТипЗначения);
				Если ОбъектМетаданных = Неопределено Тогда
					СтруктураПолей.Вставить(ЭлементКлюч, "0");
				ИначеЕсли Метаданные.Справочники.Содержит(ОбъектМетаданных) Тогда
					ЗначениеПоля =  СокрЛП(Строка(ЭлементЗначение));
					ЗначениеПоля = "" + ЭкранированиеСпециальныхСимволов(ЗначениеПоля) + "";
					СтруктураПолей.Вставить(ЭлементКлюч, ЗначениеПоля);
				ИначеЕсли Метаданные.Документы.Содержит(ОбъектМетаданных) Тогда
					ЗначениеПоля = СокрЛП(Строка(ЭлементЗначение));
					ЗначениеПоля = "" + ЭкранированиеСпециальныхСимволов(ЗначениеПоля) + "";
					СтруктураПолей.Вставить(ЭлементКлюч, ЗначениеПоля);
	...
	ТекстЗапросаПоля = ПолучитьТелоJSONИзСтруктуры(СтруктураПолей);
...
Функция ВыполнитьЗапросPOST_REST(ТекстЗапроса, Параметры)
	...
	Если Параметры.ФорматДанных = "TSV"
