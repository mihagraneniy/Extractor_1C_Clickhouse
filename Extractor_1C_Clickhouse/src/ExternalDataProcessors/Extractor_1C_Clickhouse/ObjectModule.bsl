...
Функция СведенияОВнешнейОбработке() Экспорт 
	...
	РегистрационныеДанные.Вставить("Версия", "1.3.1");
...
//
//<<%%>> ilya@kimkarus.ru @kimkarus kimkarus.ru # Ilya Kosarev 15-04-2025 9:30 start								  
Функция ПолучитьМассивЗначенийВСтрокуАпостроф(МассивЗначений)
	СтрокаМассив = ПолучитьСтрокуАпостроф(Транслит(МассивЗначений[0]));
	Количество = МассивЗначений.Количество();
	Счетчик = 1;
	Для каждого ЗначениеМ из МассивЗначений Цикл
		Если Счетчик > 1 Тогда
			СтрокаМассив = СтрокаМассив + ", " + ПолучитьСтрокуАпостроф(Транслит(ЗначениеМ));
		КонецЕсли;
		Счетчик = Счетчик + 1;	
	КонецЦикла;
	Возврат СтрокаМассив;
...
Функция ВыполнитьЗапросPOST_REST(ТекстЗапроса, Параметры)
	...
	HTTPЗапрос.УстановитьИмяФайлаТела(ФайлЗапроса);
	//
	Basic = "LOGIN_TEST" + ":" + "PASSWORD_TEST";
	Basic = Base64Строка(ПолучитьДвоичныеДанныеИзСтроки(Basic));
	//
	Заголовок = Новый Соответствие;
	Заголовок.Вставить("Authorization", "Basic " + Basic);
	Заголовок.Вставить("table_name", Параметры.ИмяТаблицыТранслит);
	Заголовок.Вставить("row_index", Параметры.ИндексЗаписи);
	Заголовок.Вставить("row_count", Параметры.КоличествоЗаписей);
	Заголовок.Вставить("guid", Параметры.ИдентификаторВыгрузки);
	HTTPЗапрос.Заголовки.Вставить(Заголовок);	
	//
	Результат = Соединение.ОтправитьДляОбработки(HTTPЗапрос);
	ТекстОтвета = Результат.ПолучитьТелоКакСтроку();
