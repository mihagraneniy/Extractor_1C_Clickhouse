...
Экстрактор данных 1С в ClickHouseDB, PostgreSQL
...
- Создать базу данных **ClickHouseDB или PostgreSQL** и выдать на нее права определенному пользователю:
...
- Должная быть открытая для соединений база **ClickHouseDB** или **PostgreSQL**.
...
Для работы требуется с **PostgreSQL**:
...
- Для замещения значений при поиске по ключевым полям используется:
  - **тип таблиц “ReplacingMergeTree” (ClickHouseDB), что означает замещение не ключевых значений новыми значениями.**
  - **метод вставки (PostgreSQL) ON CONFLICT DO UPDATE SET column1 = EXCLUDED.column1 ...**
