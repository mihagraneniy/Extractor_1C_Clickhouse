...
Для **MySQL** обычно используют **phpMyAdmin**

Для **REST** смотрим принимающую сторону, пришли ли **в теле HTTP POST** запроса данные в виде **csv с разделением через запятую**
...
- Данные отправляется в формате CSV: первая строка всегда содержит наименование колонок.
